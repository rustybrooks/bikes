    ...
    while time.time() - start_time < max_seconds:
        try:
            ...
            if response.status_code in status_forcelist:
                # Retry request
                sleep_time = min(sleep_time + 60, 300)
                logger.info(
                    "Sleeping %r for url %r (total=%0.1f)",
                    sleep_time,
                    url,
...
def redirect_token(return_to_url="/"):
    if os.getenv("ENVIRONMENT", "dev") == "local":
        ...
        redirect_uri = "https://bikes.rustybrooks.com/strava_callback/"
...
def strava_fetch(user, url, **kwargs):
    ...
    headers = {
        "api-key": str(client_id),
        "authorization": "Bearer %s" % get_auth_code(user),
    }
    # logger.warning("fetch url=%r, headers=%r", full_url, headers)
    response = retry_request(
        url=full_url, verify=True, headers=headers, max_seconds=60 * 60 * 24
    )
